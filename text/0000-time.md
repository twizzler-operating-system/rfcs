...
Rather than having a concrete type, we provide an interface implemented by different architectures for different time sources. `ClockHardware` exposes methods to read time, get a description of the hardware backing it, or adjust. This leaves more room to introduce other useful methods, such as disabling/enabling a hardware timer. 
