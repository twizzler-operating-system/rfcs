...
We define `resolution` as the period of a clock tick. In other words, the rate at which a clock advances time. We express this in `Femtoseconds` which is a simple wrapper type for a `u64` value. We choose to do this make it clear what the semantic meaning of the value is. We define `precision` as the stability of the measurements, measured in `Femtoseconds`. Another value of interest is `accuracy` which tells us how close measurements are to the true value, based on some reference. This is useful in determining clock error, and will be explored in a future RFC.
...
`Ticks` represent some duration on the clock. The width of `value`, which is 64-bits, is not fundamental and could change. The `value` can be scaled to some unit of time by multiplying the `rate` of the time source. This multiplication operation produces a `Duration`.
