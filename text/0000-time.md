...
fn main() {
    ...
    let now = sys_read_clock_info(ClockGroup::Monotonic, 0, ReadClockFlags::empty());
    
    // elapsed time since now
    let later = sys_read_clock_info(ClockGroup::Monotonic, 0, ReadClockFlags::empty());
    // ClockInfo.value() returns a Duration type
    let elapsed_time = later.value() - now.value();
    println!("Running main() took {} nanoseconds.", elapsed_time.as_nanos());
}
```
A few things here. For starters, `sys_read_clock_info`, a new system call. We pass in the type of clock we want (`ClockGroup`), an optional `clock_id`, and some flags. By default, this returns a `ClockInfo` object with all fields filled in. The returned `ClockInfo`, in this case, is generated from a `ClockGroup` specific clock. We will get to the use of `clock_id` later.
...
```rust
fn sys_read_clock_info(clock: ClockGroup, clock_id: u64, flags: ReadClockFlags) -> Result<ClockInfo, ReadClockError>
...
let now = sys_read_clock_info(ClockGroup::default() /* argument ignored */, clk.id(), ReadClockFlags::ClockID);
