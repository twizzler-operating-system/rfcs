...
Currently, these semantics only make sense for persistent objects. Objects with volatile semantics will currently treat these operations as no-ops, but this may
change during implementation (see Unresolved Questions).
...
- "Do these [Discard and ForgetWrites] APIs only make sense for pages/objects with a persistent lifetime? What are the semantics of using this on a volatile object pages?".
